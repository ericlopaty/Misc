...
namespace diff
{
    ...
    public class Diff
    {
        ...
        public static int Main(string[] args)
        {
            ...
            string left = reader.ReadToEnd();
            reader.Close();
            reader = File.OpenText(args[1]);
            string right = reader.ReadToEnd();
            reader.Close();
            reader.Dispose();
            Item[] results = Diff.DiffText(left, right, true, true, true);
            foreach (Item item in results)
                Console.WriteLine("{0} {1} {2} {3}", item.StartA, item.StartB, item.deletedA, item.insertedB);
            Console.ReadLine();
            return 0;
        }
